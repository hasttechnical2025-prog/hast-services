...
- **Office Mobile `/m`**: bản gọn cho **admin/tech_admin** (chặn staff/ktv) trên điện thoại — 4 tab: **Việc hôm nay** (tab mặc định, CHỈ XEM — nắm việc khi đi hiện trường: chỉ việc hôm nay, thẻ tổng hợp Tổng/Chưa nhận/Đang làm/Đã xong, nhóm theo trạng thái ↔ theo KTV, tìm mã máy/khách, chạm mở chi tiết read-only, realtime qua broadcast `soct_jobs`; dùng lại `GET /api/admin/cong-viec?date=hôm-nay`), Giao việc (không vật tư, bỏ Giao mực/Thay vật tư), Quét QR (→ /admin/scan), Đặt hàng (xem đơn; tech_admin tạo **đơn nháp**; ghi hàng về chỉ admin). Office trên mobile (trang chủ + Passkey) tự vào /m; PC giữ dashboard đầy đủ. KTV bị cấm /admin (login lọc role + trang /admin từ chối role ktv).
